...
# This project examines the temporal turnover of fungal endophytes in a temperate
#   fern population of the Pacific Northwest of North America, examined through
#   Illumina sequencing (culture independent). Twenty plants
#   were sampled monthly for 10 months. Four pinnae (leaflets) were sampled 
#   per plant on the same four fronds (leaves) monthly. These leaflets were surface
#   sterilized and total DNA was extracted for sequencing on the Illumina MiSeq platform.
#   Additionally, fragments of the surface sterilized leaves were plated on malt 
#   extract agar to culture fungal endophytes. From these isolates, some were
#   discovered to be the same fungi found in Illumina sequencing. We plated the most
#   prevalent fungus (OTU1) in culture with other endophytes recovered from the same plants
#   to examine their competitive ability in vitro. Lastly, we inoculated sterile fern
#   gametophytes (reared from surface sterilized spores from the same parental plants
#   in the Illumina study) with OTU1 and another OTU found from the same plants to observe
#   fungal effects on haploid ferns. Note: all files used in this code are contained 
#   in supplemental files for the manuscript.

# Outline of the code:
# 1. Load R packages, set variables and a custom theme for figure formatting
# 2. Load informatics files from Illumina project
# 3. Data cleaning and merging replicates
# 4. Rarefy to even sequencing depth
# 5. Rarefaction curves
# 6. Alpha diversity plots
# 7. Ordination plots
# 8. Taxonomy plots
# 9. Fern gametophyte fungal inoculation bioassays
# 10. In vitro competition assays
## Additional supplemental figures ##
# 11. Mock community analysis
# 12. Within-plant analysis
# 13. WP ordination plot
# 14. WP taxonomy plot
...
# ZOTUs (aka ASVs or ESVs), sample data (aka mapping file or metadata), 
#   taxonomy file
...
# Separate by month and remove all controls (will add back later)
...
# Remove the seq month April hereafter due to low read numbers, significant loss 
  # of samples after rarefaction. Likely the result of low fungal biomass 
...
# Separate by month
MAY <- subset_samples(pseq_rare_noApr, Month == 'May')
JUN <- subset_samples(pseq_rare_noApr, Month == 'Jun')
JUL <- subset_samples(pseq_rare_noApr, Month == 'Jul')
AUG <- subset_samples(pseq_rare_noApr, Month == 'Aug')
SEP <- subset_samples(pseq_rare_noApr, Month == 'Sep')
OCT <- subset_samples(pseq_rare_noApr, Month == 'Oct')
NOV <- subset_samples(pseq_rare_noApr, Month == 'Nov')
DEC <- subset_samples(pseq_rare_noApr, Month == 'Dec')
JAN <- subset_samples(pseq_rare_noApr, Month == 'Jan')

# Remove missing taxa after separating
...
# Make dfs with relevant columns
...
# ggsave('rare_v2.pdf', width = s_w, height = s_h, units = 'in')
...
# ggsave('hill_supp_v2.pdf', width = h_w, height = h_h, units = 'in')
...
shapiro.test(no_six$Hill_value)
...
p3 <- ggplot(hill2, aes(x = Month, y = Hill_value, fill = Month)) + 
...
# ggsave('hill_v5.pdf', width = s_w, height = s_h, units = 'in')
...
# ggsave('ord_v3.pdf', width = h_w, height = h_h, units = 'in')
...
p5 + geom_bar(aes(fill = Genus, color = Genus), stat = 'identity', 
...
# ggsave('tax_supp_V2.pdf', width = d_w, height = d_h, units = 'in')

# Filter low-abundance taxa
pseq_rel_filt <- filter_taxa(pseq_rel, function(x) mean(x) > 2e-5, prune = TRUE)

# Final plot for MS with filtering
p6 <- plot_bar(pseq_rel_filt, x = 'Class', fill = 'Genus') + 
...
# ggsave('tax_V2.pdf', width = h_w, height = h_h, units = 'in')

##########
# 9. Fern gametophyte fungal inoculation bioassays

# Import data
gamete <- read.csv('Nov_time1.csv', header = T, sep = "\t")

# Add new column without "_1" or "_2" in SampleIDs
gamete$Sample_ID2 <- sub('(\\w+_\\d+)_\\d', '\\1', gamete$SampleID)

# Add taxonomic names to accession numbers
gamete$SampleID[gamete$Sample_ID2 == 'BSY_163'] <- 'OTU 1'
...
# Separate by timepoint to analyze separately 
...
# Check parametric assumptions
# Week 4
qqnorm(week4$Nov_Time1)
qqline(week4$Nov_Time1)
hist(week4$Nov_Time1, breaks = 10)
bartlett.test(Nov_Time1~SampleID, data = week4)
shapiro.test(week4$Nov_Time1)

# Week 8
qqnorm(week8$Nov_Time1)
qqline(week8$Nov_Time1)
hist(week8$Nov_Time1, breaks = 10)
bartlett.test(Nov_Time1~SampleID, data = week8)
shapiro.test(week8$Nov_Time1)

week4_mod1 <- aov(Nov_Time1 ~ SampleID, data = week4)
summary(week4_mod1)

week8_mod1 <- aov(Nov_Time1 ~ SampleID, data = week8)
summary(week8_mod1)

week8$SampleID <- as.factor(week8$SampleID)
Tuk2 <- TukeyHSD(aov(Nov_Time1 ~ SampleID, data = week8), las = 1)
Tuk2
...
gamete2[1,6] <- -0.25
gamete2[31,6] <- -0.25
gamete2[1,7] <- '2.19, p = 0.131'
gamete2[31,7] <- '18.87, p < 0.001'
...
# The plot
p7 <- ggplot(gamete2, aes(x=SampleID, y=Nov_Time1, fill = SampleID)) + 
  ...
  annotate('text', x = 0.75, y = -0.25, 
           label = expression(paste(F["2,27"]," = ")), 
           hjust = hjust, size = size) + 
  annotate('text', x = 0.725, y = -0.18, label = 'ANOVA', 
           hjust = hjust, size = size) + 
...
# ggsave('gamete_V5.pdf', width = s_w, height = s_h, units = 'in')

##########
# 10. In vitro competition assays

# Load data
comp1 <- read.csv('compiled_images.csv', header = TRUE)
...
p8 <- ggplot(tow_len, aes(x = Competitor, y = Length, fill = Competitor)) + 
...
# ggsave('comp_V2.pdf', width = s_w, height = s_h, units = 'in')
...
# Substract the extraction and pcr controls from each run, leaving 
  # only seq controls (mock community samples)
...
# Make new sample data file and remerge OTUs, sample data and tax files
...
p9 <- plot_ordination(pseq_mocks_rare, ordinate(pseq_mocks_rare, 'NMDS', 
...
# ggsave('wp_ord_supp_v2.pdf', width = h_w, height = h_h, units = 'in')
...
p11 <- plot_bar(wp_filt, 'Class', fill = 'Genus')
p11 + geom_bar(aes(color = Genus, fill = Genus), stat = 'identity',
...
# ggsave('wp_tax_supp_v2.pdf', width = h_w, height = h_h, units = 'in')
